...
public class SubAreaDaoImpl implements SubAreaDao {
	...
	public Integer getId() {
		...
		Integer pr =  (Integer) jdbcTemplate.queryForObject("select case when max(id) > 0 then max(id)+1 else 1 end as valor from RS_SUBAREA_CATALOG ",
	...
	public List<Catalog> getRows(Integer areaId) {
		// TODO Auto-generated method stub
		  List<Catalog> pr =  jdbcTemplate.query("select * from RS_SUBAREA_CATALOG where status =1 and areaId=? ",
	                new Object[] {areaId  }, new BeanPropertyRowMapper<Catalog>(Catalog.class));
	...
	public void removeRow(Integer id) {
		// TODO Auto-generated method stub
		final String sql = "UPDATE RS_SUBAREA_CATALOG SET status = ?  WHERE id = ? ";
	...
	public void addRow(Catalog p) {
		// TODO Auto-generated method stub
		final String sql = "INSERT INTO RS_SUBAREA_CATALOG (id, name, status, createdAt,areaId "+")  VALUES ( ?,?,?, ?,?)";
	...
	public Catalog getDataById(Integer id) {
		...
		Catalog pr = null;
		try {
			pr =	(Catalog) jdbcTemplate.queryForObject("select * from RS_SUBAREA_CATALOG where id= ?  ",
